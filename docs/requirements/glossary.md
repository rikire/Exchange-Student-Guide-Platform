...
| Submission number | The identifier a contributor is shown when a submission is accepted into the queue, and the only handle they have on it afterwards — there is no account to look it up under. Also how the queue identifies a submission to the moderator. Because holding it is the entire authorisation, it is a **generated unguessable token, not a sequence** — [ADR-0011](../architecture/adr/ADR-0011-submission-number-format.md), measured by [NFR-006](non-functional.md) |
